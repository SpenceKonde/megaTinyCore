...
$: Ext. Clock recommended. DA-series, particularly with autotune is probably good enough though.
...
The middle 6 options can be achieved by most parts. Not all parts can make it all the way to their maximum tuning value before the chip becomes unstable; generally, they make it to 32 MHz at 5V at room temperature, but begin failing dramatically just above that; some conk out a little earlier; . I would not recommend running code at anything above 25 MHz, except to investigate the behavior of overclocked AVRs, and I would never run them above 20 MHz if a hang or crash would be inconvenient. 24 MHz is a particularly useful frequency, as that is the top rated speed for the Dx-series parts (though they can also be run a higher than rated speed - I have an extended temp range one that runs at 48 MHz off ext. clock....), and code written for 24 MHz Dx-series would run at that same speed on an overclocked tinyAVR at 24 MHz. Assuming it was stable of course
...
My idea here is simple: Run RTC from internal 32kHz oscillator. This is inaccuerate, but I belive it to be reasonably stable. Pipe it to the event system via ASYNCCH3 for 1-series or any odd channel for the 2-series (0-series parts and their owners are S. O. L. here - as usual for those low budget parts - because one of the ways in which the 0-series `EVSYS` is gimped is that, lacking async channels 2 and 3 while having the exacty same list of peripherals for the channels it does have, it can't use the divided PIT signals as event generators. What's rather hard to belive, in light of how much worse they are just to save 7 cents on the same-flash-size versions, or 15 cents on a 1607 vs a 3216) supply of the 0-series seems to be virtually nonexistant - everyplace is talking about ship times next summer, while 1-series parts, even the QFN ones that seem to be the hardest to get right now, have estimated ship dates from distributors before year-end), PIT/64 should be around 512 Hz.
