...
2. The bootloader clears only WDRF. If a classic AVR, it turns off the WDT. Other reset flags left in place. This was used by Optiboot classically, and is, for example, what the Uno does. This achieves minimum functionality but because only WDRF is cleared, it is still left to user code (which can be relied upon **not** to do it) to clear RSTFR in order not only to detect dirty resets (which I am aware of no historical practice of within the Arduino community) and decide upon entry to the bootloader. That is to say, on a classic or modern AVR, the bootloader entry conditions are honored only if the user code clears RSTFR. Most users are unaware of this register, and hence do not reset it, and thus once the reset button has been pressed once, EXTRF remains set, until a BOR or POR occurs. Note also that a poor implementation of the hardware around the reset pin can cause all POR's to be followed by an EXTR (this would result on every reset running the bootloader too). This was not as acute of an issue on classic AVRs because there were fewer reset sources. Now, we have 6 reset sources, only 2 of them reset other reset flags. Hell, this will even cause behavior after burn bootloader to differ from behavior after a power cycle.
...
|    1 |     1 |    X | The reset pin voltage rose significantly more slowly than VDD. `*`
...
`*` Misspelling of vector is intended, underline how perverse this behavior is. I have asked people knowledgeable about the workings of the compiler and received vague and condescending answers - "Obviously there's no way we can reliably detect that so we can't make an error" and throw in some jargon that I don't know. To me it is inconcievable that, while I can't assign a value to a symbol I haven't declared yet, or call a function that doesn't exist, but I can use the ISR macro, misspell the name of the vector rendering the app entirely unusable, and all I get is a warning? What?
...
Note that you don't always have empty flash after your sketch - specifically, if using the included bootloader, you won't if the previous program was bigger. That code should remain unreachable, except in this kind of situation. The results are likely to be bizarre, confusing, and very hard to rationalize.
...
#### Bad inline assembly
If you're writing assembly you should know this.... but be certain that you use the same number of push and pop instructions, and be careful to give the right constraints so the compiler can't assume that a value your changing is constant. It's very easy to forget that, say, that pointer you're passing in and reading or writing with postincrement (or predecrement) - that is not read only, and cannot be given as an input opperand - you're changing it! It should be a read-write output opperand. The compiler is gullible, if you lie to it about what you're doing with data, go along with almost anything when inline assembly is involved. The role of smacking down bad asm is left to the linker, and it only stops you from doing things that ask it to do the impossible (ie, `ldi r15,1` `adiw %0, 1` where %0 is specified with a constraint like `+r` or `+d`, and in the current program, the registers that get assigned based on those incorrect constraints (adiw needs +w or =w - or one of the even more restrictive one) and the toolchain is humming along turning asm into opcodes, and then gets to a combination of instructions and operands that doesn't have an opcode. Nothing more that it can do other than error out. But it will otherwise let you do all manner of unholy things without breaking compilation - things that will cause novel and exotic software malfunctions (but usually just a hang or a dirty reset).
