...
### AutoReset
Normal Arduino boards autoreset wbn the serial port is opened. It does this throughg the "DTR autoreset circuit". AVRdude, like any program that doesn't override the default behavior will set DTR and RTS lines low while the serial port is open. This circuit converts the transition into a pulse, allowing the chip to leave reset too. (your finer serial terminals let you control these two pins. Generally speaking DTR and RTS are interchangible, and you use whichever one the serial adapter makes available more easily).
...
If you you don't *really* know what you're doing, let the compiler manage the stack pointer. Any mistake is likely to will generally result in a dirty resert.
