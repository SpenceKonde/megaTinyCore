...
|      0x01 |     0b0000001 | Reserved for compatiblity w/CBUS protocol (obsolete)    |
|      0x02 |     0b0000010 | Reserved for compatiblity w/other bus protocols         |
...
#### Methods not available for the tinyAVR product line
These new methods are only supported on larger (full-size) parts, and support enhanced features on those parts.
```c++
...
The implementation isn't identical, but the behavior is unchanged - or differ only in an irrelevant and implicitly convertable return or argument type (some versions of the official core have sizes returned as  `size_t` for example; Nothing that any AVR will ever likely to involve single I2C-related function calls that act on 256 or more bytes; using a `uint8_t` helps to reduce flash size).
```c++
    void beginTransmission(uint8_t address);
    uint8_t requestFrom(uint8_t address, uint8_t quantity, uint8_t sendStop); // changed from size_t return type. Argument types differ, but are implicitly convertable without issues.
