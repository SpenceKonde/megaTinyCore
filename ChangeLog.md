...
* (planned) Support use of Tuned Internal oscillator to operate at 12 MHz, 12.8 MHz, 16 MHz, 20 MHz, 24 MHz, 25 MHz, 30 MHz or even 32 MHz! For many specimens, 32 MHz is just too fast, but I suspect 24 and 25 will work under favorable conditions. You must run the tuning sketch connected to a 500 Hz squarewave (Use any classic AVR **WITH A CRYSTAL** - the thin vaguely rectangular ones but with sloping edges and 3 terminals are resonators, and they are not very accurate. The rectangular ones (often with gold around the edge or at contacts) with vertical sides, with length about 1.5x width, those generally are either crystals or even full oscillators, and are suitable); Tuning values are written to the USERROW (last 16 bytes). Once tuning is completed, it is recommended to repeat for the other base oscillator (ie, 16 and 20 MHz). Yes in tuned configuration, you can run at 20 if OSCCFG fuse is pointing at 16 MHz and vise versa. The compliance of the oscillators is rather impressive....
