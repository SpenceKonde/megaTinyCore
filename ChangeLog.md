...
* **New I2C/TWI implementation** (I2C and TWI refer to the same interface, the one provided by Wire.h; don't get me started about how these and the library are named).
 ...
 [ ]Failed attempts will timeout instead of hanging indefinitely. The timeout is fixed and arbirtrary - y'all complained about compile size, and I'm not throwing that away for a timeout. We use a polling loop to see when the slave has responded, but we keep a running tally of passes through the loop, and exit the loop and return 0 (indicating no response) if we timeout.
