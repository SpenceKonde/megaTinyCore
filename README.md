...
This board package also supports using an external 32.768khz crystal as the clock source for the RTC. If this is done, make sure that the crystal is connected between the TOSC1 and TOSC2 pins, that nothing else is, that no excessively long wires or traces are connected to these pins, and that appropriate loading capacitors per crystal manufacturer datasheet are connected. A crystal without appropriate loading caps, or with long wires or traces will not oscillate at the frequency it is supposed to. Be aware that the TOSC1 and TOSC2 pins are the same pins used for serial on many parts (see part-specific pages for notes on applicable options for timer source on each part) - if this is the case, serial must be moved to the alternate set of pins.
...
### DAC Support
The 1-series parts have an 8-bit DAC which can generate a real analog voltage (note that this provides very low current and can only be used as a voltage reference, it cannot be used to power other devices). This generates voltages between 0 and the selected VREF (which cannot be VCC, unfortunately) - select the DAC VREF voltage from the Tools -> DAC Voltage Reference submenu. This voltage must be lower than Vcc to get the correct voltages. Call analogWrite() on the DAC pin to set the voltage to be output by the DAC. To turn off the DAC output, call digitalWrite() on that pin. The Tools -> DAC Voltage menu contains an additional option to disable the DAC output, this saves a bit of flash (more than you would think, because it includes .
