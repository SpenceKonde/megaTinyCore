...
// This example uses the PIN_Pxn notation, so this example will be on PORTB on all megaTinyCore boards that have a PORTB.

#define PIN PIN_PB0
//We are using the PORTB library, so the pin we use must be on PORTB.
